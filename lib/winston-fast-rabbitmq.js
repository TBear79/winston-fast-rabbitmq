...
class MessageHelper{
	static wrapMessage(applicationId, datetime, level, message){
		return JSON.stringify({ applicationId: applicationId, datetime: datetime.toISOString(), level: level, message: message  });
	}
}
...
class WinstonInstantRabbitMq{
	...
	log(level, msg, meta, callback){
		...
		if(t.wrapMessage)
			msg = MessageHelper.wrapMessage(t.applicationId, new Date(), level, msg);

		t.getConnection()
			.then((channel) => {
				return channel.assertExchange(t.amqp.exchangeName, t.amqp.exchangeType, {durable: t.amqp.durable})
					.then((ok) => {
						let publish = channel.publish(t.amqp.exchangeName, t.amqp.virtualHost, new Buffer(msg));
